...
5. "결과가 에이전트에게 신뢰할 수 있을 만큼 일관적인가?" (B2A2H 전환 전제 조건)
```

## 아키텍처 원칙 (2026-03-25 확정)
```
탐지 계층: DOM + JS 측정 (결정론적 — overflow/broken image/viewport leak)
증거 계층: 스크린샷 (물리적 시각 증거 — CEO 방침 유지)
설명 계층: AI 번역 (에이전트/사람이 읽는 리포트)

AI는 탐지자가 아니다 → 번역자다.
DOM이 먼저 재고, AI가 설명한다.
```

## 사업성·현실성 판단 원칙 (2026-03-25 추가)
```
사업성: 신뢰 없는 B2A2H는 퍼널 첫 단계부터 막힌다.
        일관성 = 수익화 전제 조건. 프롬프트로 해결 불가 → 구조 변경 필수.

현실성: DOM 레이어는 기존 인프라 재활용, 1일 구현, 롤백 비용 0.
        리스크 없는 개선. 미룰 이유 없음.

경쟁력: DOM + MCP 조합은 대형 플레이어 흡수 불가 포지션.
        스크린샷 AI만으로는 Gemini 네이티브 기능에 흡수 리스크 존재.
```
...
```
[모델 선택]
복잡하고 어려운 작업 (아키텍처·멀티파일·피벗 분석) = Claude Opus 필수
중간 수준 작업 (단일 기능·버그 수정) = Claude Sonnet
단순 조회·확인 = Claude Haiku

[병렬 작업 최우선]
독립된 태스크는 반드시 동시 실행 — 순차 실행 금지
예: 파일 읽기 여러 개 → 한 번에 / 문서 갱신 여러 개 → 동시 Edit
병렬 불가한 경우만 순차 실행 (이전 결과가 다음 입력인 경우)
```
