...
| 서비스 | Discovery? | 추천? | 설치? | ROI? | 위협도 |
|---|---|---|---|---|---|
| **VS Code MCP Discovery** | ✅ 워크스페이스 기반 | ✅ 동적 | ✅ | ❌ | **높음** ⚠️ |
| **Cline Marketplace** | ✅ 내장 | 부분 | ✅ 원클릭 | ❌ | **높음** ⚠️ |
| **Composio Tool Router** | ✅ | ✅ 태스크 기반 | ✅ | ❌ | **높음** |
| **MCP Advisor** (오픈소스) | ❌ | ✅ 시맨틱 | ❌ | ❌ | 중 |
| **Goose Extensions** (Block) | ✅ 동적 | ❌ | ✅ | ❌ | 중 |
| **MCPize** | ✅ | ❌ | ✅ | ❌ | 중 (수익화) |

### ⚠️ 최대 위협: 클라이언트 내장 마켓플레이스

**VS Code** `chat.mcp.discovery.enabled` + 동적 도구 발견 = perceptdot의 "프로젝트 분석 추천"과 가장 유사
**Cline** 내장 마켓플레이스 + MCP Rules 자동 활성화 = 수백만 유저 접근성

→ **이들과 경쟁 X, 이들 위에 올라가는 레이어(ROI 측정+추천 알고리즘)가 전략**

### 핵심 발견

1. **"발견+설치"는 이미 레드오션** — Smithery 7,300+, Glama 11,000+, Cline/VS Code 내장
2. **"프로젝트 분석 자동 추천"은 블루오션에 가까움** — VS Code 동적 발견이 가장 위협적이나 ROI 없음
3. **"ROI 측정 + 추천" 결합 = 전무** — 이것이 perceptdot의 진짜 해자
4. **B2A2H = perceptdot만의 모델** — 에이전트→사람 결제 권유 구현체 0건
5. **수익화 인프라는 이미 존재** — MCPize(85%분배), Nevermined(마이크로페이먼트) 활용 가능

### 시장 규모 (업데이트)

- MCP 서버: 11,000~18,800+개 (Glama 11,415, mcp.so 등)
- MCP 클라이언트: 542개 (PulseMCP)
- 주간 다운로드: 3,100만 (Glama 2025 리포트)
- GitHub 커밋: 85,000 (2025), 기여자 15,294명
- 생태계 펀딩: $73M+
- 시장 규모: $1.8~2.7B (2025) → $5.6B (2034)
- MCP 유료화: 5% 미만 → 거대 공백
- 리모트 서버 MoM 성장: 월 56→301개 (6개월간)
